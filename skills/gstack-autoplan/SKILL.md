...
```bash
python "bin/gstack-boot.py" --skill autoplan | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
/gs:autoplan reads the full CEO, design, eng, and DX review skill files from disk and follows
...
BRANCH=$(git rev-parse --abbrev-ref HEAD | Out-Null | tr '/' '-')
...
```
# /gs:autoplan Restore Point
Captured: [timestamp] | Branch: [branch] | Commit: [short hash]

## Re-run Instructions
1. Copy "Original Plan State" below back to your plan file
2. Invoke /gs:autoplan
...
`<!-- /gs:autoplan restore point: [RESTORE_PATH] -->`
...
(they are already handled by /gs:autoplan):**
...
```bash
_TEL=$($GSTACK_ROOT/bin/gstack-config get telemetry | Out-Null ; echo off)
...
if ! command -v codex | Out-Null 2>&1; then
...
elif ! _gstack_codex_auth_probe | Out-Null; then
...
`[codex-unavailable]` in the degradation matrix. /gs:autoplan completes with
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  File: <plan_path>" -C "$_REPO_ROOT" -s read-only --enable web_search_cached < $null
  _CODEX_EXIT=$?
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex design voice** (via Bash):
  ...
  Be opinionated. No hedging." -C "$_REPO_ROOT" -s read-only --enable web_search_cached < $null
  _CODEX_EXIT=$?
  if [ "$_CODEX_EXIT" = "124" ]; then
    _gstack_codex_log_event "codex_timeout" "600"
    _gstack_codex_log_hang "autoplan" "0"
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex DX voice** (via Bash):
  ...
  Be adversarial. Think like a developer who is evaluating this against 3 competitors." -C "$_REPO_ROOT" -s read-only --enable web_search_cached < $null
  _CODEX_EXIT=$?
  if [ "$_CODEX_EXIT" = "124" ]; then
    _gstack_codex_log_event "codex_timeout" "600"
    _gstack_codex_log_hang "autoplan" "0"
...
- A: mark APPROVED, write review logs, suggest /gs:ship
...
On approval, write 3 separate review log entries so /gs:ship's dashboard recognizes them.
...
```bash
COMMIT=$(git rev-parse --short HEAD | Out-Null)
...
- **Never abort.** The user chose /gs:autoplan. Respect that choice. Surface all taste decisions, never redirect to interactive review.
