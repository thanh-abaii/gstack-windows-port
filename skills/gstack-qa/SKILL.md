...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /gs:qa-only. (gstack)
voice-triggers:
  - "quality check"
  - "test the app"
  - "run QA"
...
```bash
python "bin/gstack-boot.py" --skill qa | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
```bash
$B status | Out-Null | grep -q "Mode: cdp" ; echo "CDP_MODE=true" ; echo "CDP_MODE=false"
```
...
"Your working tree has uncommitted changes. /gs:qa needs a clean tree so each bug fix gets its own atomic commit."
...
```bash
New-Item -ItemType Directory -Force .gstack/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.gstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ...
   setopt +o nomatch | Out-Null ; true  # zsh compat
   eval "$($GSTACK_BIN/gstack-slug | Out-Null)"
   ls -t ~/.gstack/projects/$SLUG/*-test-plan-*.md | Out-Null | head -1
   ```
...
2. **Identify affected pages/routes** from the changed files:
   ...
   **If no obvious pages/routes are identified from the diff:** Do not skip browser testing. The user invoked /gs:qa because they want browser-based verification. Fall back to Quick mode — navigate to the homepage, follow the top 5 navigation targets, check console for errors, and test any interactive elements found. Backend, config, and infrastructure changes affect app behavior — always verify the app still works.

3. **Detect the running app** — check common local dev ports:
   ```bash
   $B goto http://localhost:3000 | Out-Null ; echo "Found app on :3000" ; \
   $B goto http://localhost:4000 | Out-Null ; echo "Found app on :4000" ; \
   $B goto http://localhost:8080 | Out-Null ; echo "Found app on :8080"
   ```
...
```bash
eval "$($GSTACK_BIN/gstack-slug | Out-Null)" ; New-Item -ItemType Directory -Force ~/.gstack/projects/$SLUG
```
...
2. **Fixed bugs that were in TODOS.md** → annotate with "Fixed by /gs:qa on {branch}, {date}"
