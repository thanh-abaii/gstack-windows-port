...
/document-release when it Get-ChildItem -Recurses coverage gaps. Use when asked to "write docs",
...
```bash
python "bin/gstack-boot.py" --skill document-generate | iex
```
...
```bash
Get-ChildItem -Recurse . -type f -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.gstack/*" -not -path "./dist/*" -not -path "./build/*" -not -path "./.next/*" | head -200
```
