...
```bash
python "bin/gstack-boot.py" --skill plan-tune | iex
```
...
# /gs:plan-tune — Question Tuning + Developer Profile (v1 observational)
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   $GSTACK_ROOT/bin/gstack-developer-profile --read | Out-Null
