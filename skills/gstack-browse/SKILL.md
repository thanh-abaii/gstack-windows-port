...
```bash
python "bin/gstack-boot.py" --skill browse | iex
```
...
echo '<div class="tweet">hello</div>' > $env:TEMP\tweet.html
$B load-html $env:TEMP\tweet.html
```
...
$B screenshot $env:TEMP\out.png --selector .tweet-card
# → $env:TEMP\out.png is 2x the pixel dimensions of the element
```
...
echo '<div class="tweet-card" style="width:400px;height:200px;background:#1da1f2;color:white;padding:20px">hello</div>' > $env:TEMP\tweet.html
$B viewport 480x600 --scale 2
$B load-html $env:TEMP\tweet.html
$B screenshot $env:TEMP\out.png --selector .tweet-card
# $env:TEMP\out.png is 800x400 px, crisp (2x deviceScaleFactor).
```
