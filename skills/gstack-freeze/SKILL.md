...
```bash
python "bin/gstack-boot.py" --skill freeze | iex
```
...
# /gs:freeze — Restrict Edits to a Directory
...
echo '{"skill":"freeze","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel | Out-Null)" | Out-Null ; echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl | Out-Null ; true
```
