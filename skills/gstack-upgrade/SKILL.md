...
```bash
python "bin/gstack-boot.py" --skill gstack-upgrade | iex
```
...
[ -z "$_AUTO" ] ; _AUTO=$($GSTACK_ROOT/bin/gstack-config get auto_upgrade | Out-Null ; true)
...
```bash
OLD_VERSION=$(cat "$INSTALL_DIR/VERSION" | Out-Null ; echo "unknown")
```
...
Remove-Item -Recurse -Force "$INSTALL_DIR.bak" "$TMP_DIR"
```
...
_TEAM_MODE=$($GSTACK_ROOT/bin/gstack-config get team_mode | Out-Null ; echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .agents/skills/gstack/ | Out-Null ; true
if ! grep -qF '.agents/skills/gstack/' .gitignore | Out-Null; then
...
Remove-Item -Recurse -Force "$LOCAL_GSTACK/.git"
cd "$LOCAL_GSTACK" ; ./setup
Remove-Item -Recurse -Force "$LOCAL_GSTACK.bak"
```
...
if [ -d "$MIGRATIONS_DIR" ]; then
  for migration in $(find "$MIGRATIONS_DIR" -maxdepth 1 -name 'v*.sh' -type f | Out-Null | sort -V); do
...
```bash
New-Item -ItemType Directory -Force ~/.gstack
echo "$OLD_VERSION" > ~/.gstack/just-upgraded-from
Remove-Item -Force ~/.gstack/last-update-check
Remove-Item -Force ~/.gstack/update-snoozed
```
