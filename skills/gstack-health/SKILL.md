...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"health","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
# /gs:health -- Code Quality Dashboard
...
ls eslint.config.* .eslintrc.* .eslintrc | Out-Null | head -1 | xargs -I{} echo "LINT: eslint ."
[ -f .pylintrc ] ; [ -f pyproject.toml ] ; grep -q "pylint\|ruff" pyproject.toml | Out-Null ; echo "LINT: ruff check ."

# Test runner
[ -f package.json ] ; grep -q '"test"' package.json | Out-Null ; echo "TEST: $(node -e "console.log(JSON.parse(require('fs').readFileSync('package.json','utf8')).scripts.test)" | Out-Null)"
[ -f pyproject.toml ] ; grep -q "pytest" pyproject.toml | Out-Null ; echo "TEST: pytest"
[ -f Cargo.toml ] ; echo "TEST: cargo test"
[ -f go.mod ] ; echo "TEST: go test ./..."

# Dead code
command -v knip | Out-Null 2>&1 ; echo "DEADCODE: knip"
[ -f package.json ] ; grep -q '"knip"' package.json | Out-Null ; echo "DEADCODE: npx knip"

# Shell linting
command -v shellcheck | Out-Null 2>&1 ; ls *.sh scripts/*.sh bin/*.sh | Out-Null | head -1 | xargs -I{} echo "SHELL: shellcheck"
```
...
```bash
eval "$($GSTACK_BIN/gstack-slug | Out-Null)" ; New-Item -ItemType Directory -Force ~/.gstack/projects/$SLUG
tail -10 ~/.gstack/projects/$SLUG/health-history.jsonl | Out-Null ; echo "NO_HISTORY"
```
...
6. **Trends require history.** On first run, say "First health check -- no trend data yet. Run /gs:health again after making changes to track progress."
