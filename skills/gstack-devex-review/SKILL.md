...
description: |
  ...
  scorecard with evidence. Compares against /gs:plan-devex-review scores if they
...
```bash
python "bin/gstack-boot.py" --skill devex-review | iex
```
...
# /gs:devex-review: Live Developer Experience Audit
...
Check for prior /gs:plan-devex-review scores:

```bash
eval "$($GSTACK_ROOT/bin/gstack-slug | Out-Null)"
$GSTACK_ROOT/bin/gstack-review-read | Out-Null | grep plan-devex-review ; echo "NO_PRIOR_PLAN_REVIEW"
```
...
If /gs:plan-devex-review scores exist from the baseline check:
