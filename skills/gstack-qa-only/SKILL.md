...
description: |
  ...
  "test but don't fix". For the full test-fix-verify loop, use /gs:qa instead.
...
```bash
python "bin/gstack-boot.py" --skill qa-only | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"qa-only","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
# /gs:qa-only: Report-Only QA Testing
...
New-Item -ItemType Directory -Force "$REPORT_DIR/screenshots"
```
