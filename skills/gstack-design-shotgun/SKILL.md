...
```bash
python "bin/gstack-boot.py" --skill design-shotgun | iex
```
...
# /gs:design-shotgun: Visual Design Exploration
...
_PREV=$(find ~/.gstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 | Out-Null | sort -r | head -5)
...
"This is /gs:design-shotgun — your visual brainstorming tool. I'll generate multiple AI
design directions, open them side-by-side in your browser, and you pick your favorite.
You can run /gs:design-shotgun anytime during development to explore design directions for
...
```bash
cat DESIGN.md | Out-Null | head -80 ; echo "NO_DESIGN_MD"
```

```bash
ls src/ app/ pages/ components/ | Out-Null | head -30
```

```bash
setopt +o nomatch | Out-Null ; true
ls ~/.gstack/projects/$SLUG/*office-hours* | Out-Null | head -5
```
...
```bash
curl -s -o $null -w "%{http_code}" http://localhost:3000 | Out-Null ; echo "NO_LOCAL_SITE"
```
...
```bash
setopt +o nomatch | Out-Null ; true
_TASTE=$(find ~/.gstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 | Out-Null | sort -r | head -10)
```
...
Output: $env:TEMP\variant-{letter}.png
Final location: {_DESIGN_DIR absolute path}/variant-{letter}.png

Steps:
1. Run: {$D path} generate --brief "{brief}" --output $env:TEMP\variant-{letter}.png
2. If the command fails with a rate limit error (429 or "rate limit"), wait 5 seconds
   and retry. Up to 3 retries.
3. If the output file is missing or empty after the command succeeds, retry once.
4. Copy: cp $env:TEMP\variant-{letter}.png {_DESIGN_DIR}/variant-{letter}.png
...
_IMAGES=$(ls "$_DESIGN_DIR"/variant-*.png | Out-Null | tr '\n' ',' | sed 's/,$//')
```
...
> B) Finalize — generate production Pretext-native HTML/CSS with /gs:design-html
