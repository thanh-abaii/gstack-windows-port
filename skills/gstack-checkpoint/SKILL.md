...
```bash
python "bin/gstack-boot.py" --skill checkpoint | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"checkpoint","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
# /gs:checkpoint — Save and Resume Working State
...
if [ -n "$_TEL_START" ]; then
  ...
  START_EPOCH=$(ps -o lstart= -p $PPID | Out-Null | xargs -I{} date -jf "%c" "{}" "+%s" | Out-Null ; echo "")
fi
...
if [ -d "$CHECKPOINT_DIR" ]; then
  find "$CHECKPOINT_DIR" -maxdepth 1 -name "*.md" -type f | Out-Null | xargs ls -1t | Out-Null | head -20
else
  echo "NO_CHECKPOINTS"
fi
```
...
if [ -d "$CHECKPOINT_DIR" ]; then
  echo "CHECKPOINT_DIR=$CHECKPOINT_DIR"
  find "$CHECKPOINT_DIR" -maxdepth 1 -name "*.md" -type f | Out-Null | xargs ls -1t | Out-Null
else
  echo "NO_CHECKPOINTS"
fi
```
