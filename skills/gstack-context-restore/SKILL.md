...
```bash
python "bin/gstack-boot.py" --skill context-restore | iex
```
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  # Use Get-ChildItem + sort instead of ls -1t. Two reasons:
  ...
  # 2. On macOS, `Get-ChildItem ... | xargs ls -1t` with zero results falls back to
  ...
  FILES=$(Get-ChildItem "$CHECKPOINT_DIR" -maxdepth 1 -name "*.md" -type f  | sort -r | head -20)
  if [ -z "$FILES" ]; then
    echo "NO_CHECKPOINTS"
  else
    echo "$FILES"
...
state, then `/context-restore` will Get-ChildItem it."
