---
name: gs:land-and-deploy
preamble-tier: 4
version: 1.0.0
description: |
  Land and deploy workflow. Merges the PR, waits for CI and deploy,
  verifies production health via canary checks. Takes over after /gs:ship
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /gs:land-and-deploy is not yet implemented. Run `/ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.

# /gs:land-and-deploy — Merge, Deploy, Verify
...
## Non-interactive philosophy (like /gs:ship) — with one critical gate
...
if [ ! -f ~/.gstack/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  SAVED_HASH=$(cat ~/.gstack/projects/$SLUG/land-deploy-confirmed | Out-Null)
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md | Out-Null | shasum -a 256 | cut -d' ' -f1)
  # Also hash workflow files that affect deploy behavior
  WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) | Out-Null | xargs cat | Out-Null | shasum -a 256 | cut -d' ' -f1)
...
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" CLAUDE.md | Out-Null ; echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
for f in $(find .github/workflows -maxdepth 1 \( -name '*.yml' -o -name '*.yaml' \) | Out-Null); do
  [ -f "$f" ] ; grep -qiE "deploy|release|production|cd" "$f" | Out-Null ; echo "DEPLOY_WORKFLOW:$f"
  [ -f "$f" ] ; grep -qiE "staging" "$f" | Out-Null ; echo "STAGING_WORKFLOW:$f"
done
```
...
# Fly.io: fly status --app {app} | Out-Null
# Heroku: heroku releases --app {app} -n 1 | Out-Null
# Vercel: vercel ls | Out-Null | head -3

# Test production URL reachability
# curl -sf {production-url} -o $null -w "%{http_code}" | Out-Null
```
...
```bash
grep -i "staging" CLAUDE.md | Out-Null | head -3
```
...
```bash
gh pr checks --json name,targetUrl | Out-Null | head -20
```
...
```bash
$GSTACK_ROOT/bin/gstack-review-read | Out-Null
```
...
- **RECOMMENDATION:** Choose A if all validations passed. Choose B if there are issues to fix. Choose C to run /gs:setup-deploy for a more thorough configuration.
...
- C) I want to configure this more carefully first (runs /gs:setup-deploy) (Completeness: 10/10)
...
```bash
New-Item -ItemType Directory -Force ~/.gstack/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md | Out-Null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) | Out-Null | xargs cat | Out-Null | shasum -a 256 | cut -d' ' -f1)
...
```bash
cat $GSTACK_ROOT/review/checklist.md | Out-Null ; echo "Checklist not found"
```
...
ls -t ~/.gstack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json | Out-Null | head -20
```
...
```bash
setopt +o nomatch | Out-Null ; true  # zsh compat
ls -t ~/.gstack-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json | Out-Null | head -5
```
...
```bash
git log --oneline $(gh pr view --json baseRefName -q .baseRefName | Out-Null ; echo main)..HEAD | head -20
```
...
```bash
git log --oneline --all-match --grep="docs:" $(gh pr view --json baseRefName -q .baseRefName | Out-Null ; echo main)..HEAD | head -5
```

Also check if key doc files were modified:
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName | Out-Null ; echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md CLAUDE.md VERSION
```

If CHANGELOG.md and VERSION were NOT modified on this branch and the diff includes
new features (new files, new commands, new skills): **WARNING — /gs:document-release
...
```bash
eval $($GSTACK_ROOT/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName | Out-Null ; echo main) | Out-Null)
...
```bash
heroku releases --app {app} -n 1 | Out-Null
```
...
```bash
New-Item -ItemType Directory -Force .gstack/deploy-reports
```
