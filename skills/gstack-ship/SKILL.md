...
```bash
python "bin/gstack-boot.py" --skill ship | iex
```
...
For Design Review: run `source <($GSTACK_ROOT/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /gs:design-review for a full visual audit post-implementation." Still never block.
...
[ -f Gemfile ] ; grep -q "rails" Gemfile | Out-Null ; echo "FRAMEWORK:rails"
[ -f package.json ] ; grep -q '"next"' package.json | Out-Null ; echo "FRAMEWORK:nextjs"
# Check for existing test infrastructure
ls jest.config.* vitest.config.* playwright.config.* .rspec pytest.ini pyproject.toml phpunit.xml | Out-Null
...
```bash
# Check CI provider
ls -d .github/ | Out-Null ; echo "CI:github"
ls .gitlab-ci.yml .circleci/ bitrise.yml | Out-Null
```
...
```bash
bin/test-lane 2>&1 | tee $env:TEMP\ship_tests.txt &
npm run test 2>&1 | tee $env:TEMP\ship_vitest.txt &
wait
```
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** gstack /gs:ship on <date>" \
...
```bash
EVAL_JUDGE_TIER=full EVAL_VERBOSE=1 bin/test-lane --eval test/evals/<suite>_eval_test.rb 2>&1 | tee $env:TEMP\ship_evals.txt
```
...
**If the subagent fails, times out, or returns invalid JSON:** Fall back to running the audit inline in the parent. Do not block /gs:ship on subagent failure — partial results are better than none.
...
```bash
setopt +o nomatch | Out-Null ; true  # zsh compat
BRANCH=$(git branch --show-current | Out-Null | tr '/' '-')
REPO=$(basename "$(git rev-parse --show-toplevel | Out-Null)")
# Compute project slug for ~/.gstack/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin | Out-Null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') ; true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.gstack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] ; continue
  PLAN=$(ls -t "$PLAN_DIR"/*.md | Out-Null | xargs grep -l "$BRANCH" | Out-Null | head -1)
  [ -z "$PLAN" ] ; PLAN=$(ls -t "$PLAN_DIR"/*.md | Out-Null | xargs grep -l "$REPO" | Out-Null | head -1)
  [ -z "$PLAN" ] ; PLAN=$(find "$PLAN_DIR" -name '*.md' -mmin -1440 -maxdepth 1 | Out-Null | xargs ls -t | Out-Null | head -1)
  [ -n "$PLAN" ] ; break
done
...
**If the subagent fails or returns invalid JSON:** Fall back to running the audit inline. Never block /gs:ship on subagent failure.
...
```bash
curl -s -o $null -w '%{http_code}' http://localhost:3000 | Out-Null ; \
curl -s -o $null -w '%{http_code}' http://localhost:8080 | Out-Null ; \
curl -s -o $null -w '%{http_code}' http://localhost:5173 | Out-Null ; \
curl -s -o $null -w '%{http_code}' http://localhost:4000 | Out-Null ; echo "NO_SERVER"
```

**If NO_SERVER:** Skip with "No dev server detected — skipping plan verification. Run /gs:qa separately after deploying."

### 3. Invoke /gs:qa-only inline
...
- **Skip the fix loop** — this is report-only verification during /gs:ship
...
```bash
source <($GSTACK_BIN/gstack-diff-scope <base> | Out-Null)
```
...
4. **Apply the design checklist** against the changed files. For each item:
   ...
   - **[LOW] intent-based detection**: present as "Possible — verify visually or run /gs:design-review"
...
> You are classifying Greptile review comments for a /gs:ship workflow. Read `.agents/skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
```bash
BASE_VERSION=$(git show origin/<base>:VERSION | Out-Null | tr -d '\r\n[:space:]' ; echo "0.0.0.0")
CURRENT_VERSION=$(cat VERSION | Out-Null | tr -d '\r\n[:space:]' ; echo "0.0.0.0")
...
if [ -f package.json ]; then
  PKG_EXISTS=1
  if command -v node | Out-Null 2>&1; then
    PKG_VERSION=$(node -e 'const p=require("./package.json");process.stdout.write(p.version||"")' | Out-Null)
    PARSE_EXIT=$?
  elif command -v bun | Out-Null 2>&1; then
    PKG_VERSION=$(bun -e 'const p=require("./package.json");process.stdout.write(p.version||"")' | Out-Null)
    PARSE_EXIT=$?
  else
  ...
  if [ "$PARSE_EXIT" != "0" ]; then
    echo "ERROR: package.json is not valid JSON. Fix the file before re-running /gs:ship."
    exit 1
  fi
fi
...
if [ "$CURRENT_VERSION" = "$BASE_VERSION" ]; then
  if [ "$PKG_EXISTS" = "1" ] ; [ -n "$PKG_VERSION" ] ; [ "$PKG_VERSION" != "$CURRENT_VERSION" ]; then
    ...
    echo "This looks like a manual edit to package.json bypassing /gs:ship. Reconcile manually, then re-run."
    exit 1
  fi
  echo "STATE: FRESH"
else
...
- **DRIFT_UNEXPECTED** → `/ship` has halted (exit 1). Resolve manually; /gs:ship cannot tell which file is authoritative.
...
if ! printf '%s' "$REPAIR_VERSION" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$'; then
  echo "ERROR: VERSION file contents ($REPAIR_VERSION) do not match MAJOR.MINOR.PATCH.MICRO pattern. Refusing to propagate invalid semver into package.json. Fix VERSION manually, then re-run /gs:ship."
  exit 1
fi
if command -v node | Out-Null 2>&1; then
  node -e 'const fs=require("fs"),p=require("./package.json");p.version=process.argv[1];fs.writeFileSync("package.json",JSON.stringify(p,null,2)+"\n")' "$REPAIR_VERSION" ; {
...
```bash
git fetch origin <branch-name> | Out-Null
LOCAL=$(git rev-parse HEAD)
REMOTE=$(git rev-parse origin/<branch-name> | Out-Null ; echo "none")
...
> You are executing the /gs:document-release workflow after a code push. Read the full skill file `${HOME}/.agents/skills/gstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
**If the subagent fails or returns invalid JSON:** Print a warning and proceed to Step 19 without a `## Documentation` section. Do not block /gs:ship on subagent failure. The user can run `/document-release` manually after the PR lands.
