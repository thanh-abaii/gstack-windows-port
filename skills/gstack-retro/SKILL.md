...
```bash
python "bin/gstack-boot.py" --skill retro | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
# /gs:retro — Weekly Engineering Retrospective
...
```
Usage: /gs:retro [window | compare | global]
  /gs:retro              — last 7 days (default)
  /gs:retro 24h          — last 24 hours
  /gs:retro 14d          — last 14 days
  /gs:retro 30d          — last 30 days
  /gs:retro compare      — compare this period vs prior period
  /gs:retro compare 14d  — compare with explicit window
  /gs:retro global       — cross-project retro across all AI tools (7d default)
  /gs:retro global 14d   — cross-project retro with explicit window
```
...
cat ~/.gstack/greptile-history.md | Out-Null ; true

# 9. TODOS.md backlog (if available)
cat TODOS.md | Out-Null ; true

# 10. Test file count
find . -name '*.test.*' -o -name '*.spec.*' -o -name '*_test.*' -o -name '*_spec.*' | Out-Null | grep -v node_modules | wc -l

# 11. Regression test commits in window
git log origin/<default> --since="<window>" --oneline --grep="test(qa):" --grep="test(design):" --grep="test: coverage"

# 12. gstack skill usage telemetry (if available)
cat ~/.gstack/analytics/skill-usage.jsonl | Out-Null ; true
...
```
| Skill Usage | /gs:ship(12) /gs:qa(8) /gs:review(5) · 3 safety hook fires |
```
...
```
  EUREKA /gs:office-hours (branch: garrytan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /gs:plan-eng-review (branch: garrytan/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
ls -t .context/retros/*.json | Out-Null
```
...
```bash
New-Item -ItemType Directory -Force .context/retros
```
...
today=$(date +%Y-%m-%d)
existing=$(ls .context/retros/${today}-*.json | Out-Null | wc -l | tr -d ' ')
next=$((existing + 1))
...
Check review JSONL logs for plan completion data from /gs:ship runs this period:

```bash
setopt +o nomatch | Out-Null ; true  # zsh compat
eval "$($GSTACK_ROOT/bin/gstack-slug | Out-Null)"
cat ~/.gstack/projects/$SLUG/*-reviews.jsonl | Out-Null | grep '"skill":"ship"' | grep '"plan_items_total"' ; echo "NO_PLAN_DATA"
```
...
[ -z "$DISCOVER_BIN" ] ; which gstack-global-discover | Out-Null 2>&1 ; DISCOVER_BIN=$(which gstack-global-discover)
...
```bash
$DISCOVER_BIN --since "<window>" --format json 2>$env:TEMP\gstack-discover-stderr
```
...
```bash
git -C <path> fetch origin --quiet | Out-Null
```
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  E.g., "Built /gs:retro global — cross-project retrospective with AI session discovery"
  not "feat: gstack-global-discover" + "feat: /gs:retro global template".
...
```bash
setopt +o nomatch | Out-Null ; true  # zsh compat
ls -t ~/.gstack/retros/global-*.json | Out-Null | head -5
```
...
```bash
New-Item -ItemType Directory -Force ~/.gstack/retros
```
...
setopt +o nomatch | Out-Null ; true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.gstack/retros/global-${today}-*.json | Out-Null | wc -l | tr -d ' ')
next=$((existing + 1))
```
