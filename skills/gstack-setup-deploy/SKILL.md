---
name: gs:setup-deploy
preamble-tier: 2
version: 1.0.0
description: |
  Configure deployment settings for /gs:land-and-deploy. Detects your deploy
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"setup-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
# /gs:setup-deploy — Configure Deployment for gstack
...
```bash
grep -A 20 "## Deploy Configuration" CLAUDE.md | Out-Null ; echo "NO_CONFIG"
```
...
for f in $(find .github/workflows -maxdepth 1 \( -name '*.yml' -o -name '*.yaml' \) | Out-Null); do
  [ -f "$f" ] ; grep -qiE "deploy|release|production|staging|cd" "$f" | Out-Null ; echo "DEPLOY_WORKFLOW:$f"
done

# Project type
[ -f package.json ] ; grep -q '"bin"' package.json | Out-Null ; echo "PROJECT_TYPE:cli"
find . -maxdepth 1 -name '*.gemspec' | Out-Null | grep -q . ; echo "PROJECT_TYPE:library"
```
...
merge to main, Render picks it up automatically. The "deploy wait" in /gs:land-and-deploy
...
3. **How can gstack check if a deploy succeeded?**
   - A) HTTP health check at a specific URL (e.g., /gs:health, /api/status)
...
```markdown
## Deploy Configuration (configured by /gs:setup-deploy)
- Platform: {platform}
- Production URL: {url}
...
```bash
curl -sf "{health-check-url}" -o $null -w "%{http_code}" | Out-Null ; echo "UNREACHABLE"
```

2. If a deploy status command was configured, try it:
```bash
{deploy-status-command} | Out-Null | head -5 ; echo "COMMAND_FAILED"
```
...
Saved to CLAUDE.md. /gs:land-and-deploy will use these settings automatically.

Next steps:
- Run /gs:land-and-deploy to merge and deploy your current PR
- Edit the "## Deploy Configuration" section in CLAUDE.md to change settings
- Run /gs:setup-deploy again to reconfigure
```
...
- **Idempotent.** Running /gs:setup-deploy multiple times overwrites the previous config cleanly.
