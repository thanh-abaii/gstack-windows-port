...
```bash
python "bin/gstack-boot.py" --skill cso | iex
```
...
ls requirements.txt pyproject.toml setup.py | Out-Null ; echo "STACK: Python"
ls go.mod | Out-Null ; echo "STACK: Go"
ls Cargo.toml | Out-Null ; echo "STACK: Rust"
ls pom.xml build.gradle | Out-Null ; echo "STACK: JVM"
ls composer.json | Out-Null ; echo "STACK: PHP"
find . -maxdepth 1 \( -name '*.csproj' -o -name '*.sln' \) | Out-Null | grep -q . ; echo "STACK: .NET"
```
...
grep -q "fastify" package.json | Out-Null ; echo "FRAMEWORK: Fastify"
grep -q "hono" package.json | Out-Null ; echo "FRAMEWORK: Hono"
grep -q "django" requirements.txt pyproject.toml | Out-Null ; echo "FRAMEWORK: Django"
grep -q "fastapi" requirements.txt pyproject.toml | Out-Null ; echo "FRAMEWORK: FastAPI"
grep -q "flask" requirements.txt pyproject.toml | Out-Null ; echo "FRAMEWORK: Flask"
grep -q "rails" Gemfile | Out-Null ; echo "FRAMEWORK: Rails"
grep -q "gin-gonic" go.mod | Out-Null ; echo "FRAMEWORK: Gin"
grep -q "spring-boot" pom.xml build.gradle | Out-Null ; echo "FRAMEWORK: Spring Boot"
grep -q "laravel" composer.json | Out-Null ; echo "FRAMEWORK: Laravel"
```
...
find . -maxdepth 4 -name "Dockerfile*" -o -name "docker-compose*.yml" | Out-Null
find . -maxdepth 4 -name "*.tf" -o -name "*.tfvars" -o -name "kustomization.yaml" | Out-Null
ls .env .env.* | Out-Null
```
...
git log -p --all -G "ghp_|gho_|github_pat_" | Out-Null
git log -p --all -G "xoxb-|xoxp-|xapp-" | Out-Null
git log -p --all -G "password|secret|token|api_key" -- "*.env" "*.yml" "*.json" "*.conf" | Out-Null
```
...
```bash
New-Item -ItemType Directory -Force .gstack/security-reports
```
...
**This tool is not a substitute for a professional security audit.** /gs:cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /gs:cso as
...
**Always include this disclaimer at the end of every /gs:cso report output.**
