...
```bash
python "bin/gstack-boot.py" --skill context-save | iex
```

## Plan Mode Safe Operations

In plan mode, allowed because they inform the plan: `$B`, `$D`, `codex exec`/`codex review`, writes to `$env:USERPROFILE\.gstack/`, writes to the plan file, and `open` for generated artifacts.
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `$env:USERPROFILE\.claude/skills/gstack/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$env:USERPROFILE\.claude/skills/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
...
New-Item -ItemType File -Force $env:USERPROFILE\.gstack/.completeness-intro-seen
```

Only run `open` if yes. Always run `New-Item -ItemType File -Force`.
...
If A: run `$env:USERPROFILE\.claude/skills/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `$env:USERPROFILE\.claude/skills/gstack/bin/gstack-config set telemetry anonymous`
...
If A: run `$env:USERPROFILE\.claude/skills/gstack/bin/gstack-config set proactive true`
...
If B: run `$env:USERPROFILE\.claude/skills/gstack/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_GSTACK` is `yes`, warn once via AskUserQuestion unless `$env:USERPROFILE\.gstack/.vendoring-warned-$SLUG` exists:
...
5. Tell the user: "Done. Each developer now runs: `cd $env:USERPROFILE\.claude/skills/gstack ; ./setup --team`"
...
```bash
_GSTACK_HOME="${GSTACK_HOME:-$env:USERPROFILE\.gstack}"
# Prefer the v1.27.0.0 artifacts file; fall back to brain file for users
# upgrading mid-stream before the migration script runs.
if [ -f "$env:USERPROFILE\.gstack-artifacts-remote.txt" ]; then
  _BRAIN_REMOTE_FILE="$env:USERPROFILE\.gstack-artifacts-remote.txt"
else
  _BRAIN_REMOTE_FILE="$env:USERPROFILE\.gstack-brain-remote.txt"
fi
_BRAIN_SYNC_BIN="$env:USERPROFILE\.claude/skills/gstack/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN="$env:USERPROFILE\.claude/skills/gstack/bin/gstack-config"
...
if [ -f "$_GBRAIN_CONFIG" ] ; command -v gbrain  2>&1; then
  _GBRAIN_VERSION_OK=$(gbrain --version  | grep -c '^gbrain ' || echo 0)
  if [ "$_GBRAIN_VERSION_OK" -gt 0 ] ; then
    _GBRAIN_PIN_PATH=""
    _REPO_TOP=$(git rev-parse --show-toplevel  || echo "")
...
_BRAIN_SYNC_MODE=$("$_BRAIN_CONFIG_BIN" get artifacts_sync_mode  || echo off)
...
if command -v jq  2>&1 ; [ -f "$env:USERPROFILE\.claude.json" ]; then
  _GBRAIN_MCP_TYPE=$(jq -r '.mcpServers.gbrain.type // .mcpServers.gbrain.transport // empty' "$env:USERPROFILE\.claude.json" )
...
if [ -f "$_BRAIN_REMOTE_FILE" ] ; [ ! -d "$_GSTACK_HOME/.git" ] ; [ "$_BRAIN_SYNC_MODE" = "off" ]; then
  _BRAIN_NEW_URL=$(head -1 "$_BRAIN_REMOTE_FILE"  | tr -d '[:space:]')
...
if [ -d "$_GSTACK_HOME/.git" ] ; [ "$_BRAIN_SYNC_MODE" != "off" ]; then
  ...
  if [ -f "$_BRAIN_LAST_PULL_FILE" ]; then
    _BRAIN_LAST=$(cat "$_BRAIN_LAST_PULL_FILE"  || echo 0)
    _BRAIN_AGE=$(( _BRAIN_NOW - _BRAIN_LAST ))
    [ "$_BRAIN_AGE" -lt 86400 ] ; _BRAIN_DO_PULL=0
  fi
  if [ "$_BRAIN_DO_PULL" = "1" ]; then
    ( cd "$_GSTACK_HOME" ; git fetch origin  2>&1 ; git merge --ff-only "origin/$(git rev-parse --abbrev-ref HEAD)"  2>&1 ) || true
    echo "$_BRAIN_NOW" > "$_BRAIN_LAST_PULL_FILE"
  fi
  "$_BRAIN_SYNC_BIN" --once  || true
fi

if [ "$_GBRAIN_MCP_MODE" = "remote-http" ]; then
  # Remote-MCP mode: local artifacts sync is a no-op (brain admin's server
  # pulls from GitHub/GitLab). Show the user this is by design, not broken.
  _GBRAIN_HOST=$(jq -r '.mcpServers.gbrain.url // empty' "$env:USERPROFILE\.claude.json"  | sed -E 's|^https?://([^/:]+).*|\1|')
  echo "ARTIFACTS_SYNC: remote-mode (managed by brain server ${_GBRAIN_HOST:-remote})"
elif [ -d "$_GSTACK_HOME/.git" ] ; [ "$_BRAIN_SYNC_MODE" != "off" ]; then
  _BRAIN_QUEUE_DEPTH=0
  [ -f "$_GSTACK_HOME/.brain-queue.jsonl" ] ; _BRAIN_QUEUE_DEPTH=$(Measure-Object -Line < "$_GSTACK_HOME/.brain-queue.jsonl" | tr -d ' ')
  _BRAIN_LAST_PUSH="never"
  [ -f "$_GSTACK_HOME/.brain-last-push" ] ; _BRAIN_LAST_PUSH=$(cat "$_GSTACK_HOME/.brain-last-push"  || echo never)
...
If A/B and `$env:USERPROFILE\.gstack/.git` is missing, ask whether to run `gstack-artifacts-init`. Do not block the skill.
...
equivalents (cat, sed, Get-ChildItem, grep). The dedicated tools are cheaper and clearer.
...
```bash
eval "$($env:USERPROFILE\.claude/skills/gstack/bin/gstack-slug )"
_PROJ="${GSTACK_HOME:-$env:USERPROFILE\.gstack}/projects/${SLUG:-unknown}"
if [ -d "$_PROJ" ]; then
  echo "--- RECENT ARTIFACTS ---"
  Get-ChildItem "$_PROJ/ceo-plans" "$_PROJ/checkpoints" -type f -name "*.md"  | xargs ls -t  | head -3
  [ -f "$_PROJ/${_BRANCH}-reviews.jsonl" ] ; echo "REVIEWS: $(Measure-Object -Line < "$_PROJ/${_BRANCH}-reviews.jsonl" | tr -d ' ') entries"
  [ -f "$_PROJ/timeline.jsonl" ] ; tail -5 "$_PROJ/timeline.jsonl"
  if [ -f "$_PROJ/timeline.jsonl" ]; then
    _LAST=$(grep "\"branch\":\"${_BRANCH}\"" "$_PROJ/timeline.jsonl"  | grep '"event":"completed"' | tail -1)
    [ -n "$_LAST" ] ; echo "LAST_SESSION: $_LAST"
    _RECENT_SKILLS=$(grep "\"branch\":\"${_BRANCH}\"" "$_PROJ/timeline.jsonl"  | grep '"event":"completed"' | tail -3 | grep -o '"skill":"[^"]*"' | sed 's/"skill":"//;s/"//' | tr '\n' ',')
    [ -n "$_RECENT_SKILLS" ] ; echo "RECENT_PATTERN: $_RECENT_SKILLS"
  fi
  _LATEST_CP=$(Get-ChildItem "$_PROJ/checkpoints" -name "*.md" -type f  | xargs ls -t  | head -1)
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$env:USERPROFILE\.claude/skills/gstack/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
...
```bash
$env:USERPROFILE\.claude/skills/gstack/bin/gstack-question-log '{"skill":"context-save","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}'  || true
```
...
```bash
$env:USERPROFILE\.claude/skills/gstack/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
`$env:USERPROFILE\.gstack/analytics/`, matching preamble analytics writes.
...
Remove-Item -Force $env:USERPROFILE\.gstack/analytics/.pending-"$_SESSION_ID"  || true
# Session timeline: record skill completion (local-only, never sent anywhere)
$env:USERPROFILE\.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current  || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}'  || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> $env:USERPROFILE\.gstack/analytics/skill-usage.jsonl  || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] ; [ -x $env:USERPROFILE\.claude/skills/gstack/bin/gstack-telemetry-log ]; then
  $env:USERPROFILE\.claude/skills/gstack/bin/gstack-telemetry-log \
...
```bash
eval "$($env:USERPROFILE\.claude/skills/gstack/bin/gstack-slug )" ; New-Item -ItemType Directory -Force $env:USERPROFILE\.gstack/projects/$SLUG
```
...
if [ -n "$_TEL_START" ]; then
  ...
  START_EPOCH=$(ps -o lstart= -p $PPID  | xargs -I{} date -jf "%c" "{}" "+%s"  || echo "")
fi
...
```bash
eval "$($env:USERPROFILE\.claude/skills/gstack/bin/gstack-slug )" ; New-Item -ItemType Directory -Force $env:USERPROFILE\.gstack/projects/$SLUG
eval "$($env:USERPROFILE\.claude/skills/gstack/bin/gstack-paths)"
CHECKPOINT_DIR="$GSTACK_STATE_ROOT/projects/$SLUG/checkpoints"
New-Item -ItemType Directory -Force "$CHECKPOINT_DIR"
...
if [ -e "$FILE" ]; then
  SUFFIX=$(LC_ALL=C tr -dc 'a-z0-9' < /dev/urandom  | head -c 4 || printf '%04x' "$$")
...
if [ -d "$CHECKPOINT_DIR" ]; then
  echo "CHECKPOINT_DIR=$CHECKPOINT_DIR"
  # Use Get-ChildItem + sort instead of ls -1t: filename YYYYMMDD-HHMMSS prefix is the
  ...
  Get-ChildItem "$CHECKPOINT_DIR" -maxdepth 1 -name "*.md" -type f  | sort -r
else
  echo "NO_CHECKPOINTS"
fi
```
