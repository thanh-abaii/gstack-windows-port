...
```bash
python "bin/gstack-boot.py" --skill benchmark | iex
```
...
# /gs:benchmark — Performance Regression Detection
...
New-Item -ItemType Directory -Force .gstack/benchmark-reports
New-Item -ItemType Directory -Force .gstack/benchmark-reports/baselines
```

### Phase 2: Page Discovery

Same as /gs:canary — auto-discover from navigation or use `--pages`.

If `--diff` mode:

```bash
git diff $(gh pr view --json baseRefName -q .baseRefName | Out-Null ; gh repo view --json defaultBranchRef -q .defaultBranchRef.name | Out-Null ; echo main)...HEAD --name-only
```
