...
description: |
  ...
  of truth. For existing sites, use /gs:plan-design-review to infer the system instead.
...
```bash
python "bin/gstack-boot.py" --skill design-consultation | iex
```
...
# /gs:design-consultation: Your Design System, Built Together
...
```bash
ls DESIGN.md design-system.md | Out-Null ; echo "NO_DESIGN_FILE"
```
...
```bash
cat README.md | Out-Null | head -50
cat package.json | Out-Null | head -20
...
ls .context/*office-hours* .context/attachments/*office-hours* | Out-Null | head -5
```
...
$B screenshot "$env:TEMP\design-research-site-name.png"
$B snapshot
```
...
```bash
PREVIEW_FILE="$env:TEMP\design-consultation-preview-$(date +%s).html"
```
...
|------|----------|-----------|
| [today] | Initial design system created | Created by /gs:design-consultation based on [product context / research] |
```
...
"Want to see this design system as working Pretext-native HTML? Run /gs:design-html."
