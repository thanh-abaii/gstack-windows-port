...
```bash
python "bin/gstack-boot.py" --skill learn | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
$GSTACK_ROOT/bin/gstack-learnings-search --limit 20 | Out-Null ; echo "No learnings yet."
```

Present the output in a readable format. If no learnings exist, tell the user:
"No learnings recorded yet. As you use /gs:review, /gs:ship, /gs:investigate, and other skills,
...
```bash
eval "$($GSTACK_ROOT/bin/gstack-slug | Out-Null)"
$GSTACK_ROOT/bin/gstack-learnings-search --query "USER_QUERY" --limit 20 | Out-Null ; echo "No matches."
```
...
```bash
eval "$($GSTACK_ROOT/bin/gstack-slug | Out-Null)"
$GSTACK_ROOT/bin/gstack-learnings-search --limit 100 | Out-Null
```
...
```bash
eval "$($GSTACK_ROOT/bin/gstack-slug | Out-Null)"
$GSTACK_ROOT/bin/gstack-learnings-search --limit 50 | Out-Null
```
