...
```bash
python "bin/gstack-boot.py" --skill canary | iex
```
...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"canary","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' | Out-Null ; true
```
...
# /gs:canary — Post-Deploy Visual Monitor
...
```bash
eval "$($GSTACK_ROOT/bin/gstack-slug | Out-Null ; echo "SLUG=unknown")"
New-Item -ItemType Directory -Force .gstack/canary-reports
New-Item -ItemType Directory -Force .gstack/canary-reports/baselines
New-Item -ItemType Directory -Force .gstack/canary-reports/screenshots
```
