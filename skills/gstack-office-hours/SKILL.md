...
```bash
python "bin/gstack-boot.py" --skill office-hours | iex
```
...
After understanding the problem through questioning, search for what the world thinks. This is NOT competitive research (that's /gs:design-consultation's job). This is understanding conventional wisdom so you can evaluate where it's wrong.
...
```bash
echo '{"approved_variant":"<VARIANT>","feedback":"<FEEDBACK>","date":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","screen":"mockup","branch":"'$(git branch --show-current | Out-Null)'"}' > "$_DESIGN_DIR/approved.json"
```
...
$B screenshot $env:TEMP\gstack-sketch.png
```
...
```bash
which codex | Out-Null ; echo "CODEX_AVAILABLE" ; echo "CODEX_NOT_AVAILABLE"
```
...
codex exec "For this product approach, provide: a visual thesis (one sentence — mood, material, energy), a content plan (hero → support → detail → CTA), and 2 interaction ideas that change page feel. Apply beautiful defaults: composition-first, brand-first, cardless, poster not document. Be opinionated." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached < $null 2>"$TMPERR_SKETCH"
```
...
```bash
New-Item -ItemType Directory -Force "${GSTACK_HOME:-$HOME/.gstack}"
```
...
**"Design doc saved to: {full path}. Other skills (/plan-ceo-review, /gs:plan-eng-review) will find it automatically."**
...
Generated by /gs:office-hours on {date}
Branch: {branch}
Repo: {owner/repo}
Status: DRAFT
Mode: Startup
...
```bash
PROFILE=$($GSTACK_ROOT/bin/gstack-builder-profile | Out-Null) ; PROFILE="SESSION_COUNT: 0
TIER: introduction"
