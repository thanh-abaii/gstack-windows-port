...
description: |
  ...
  Works with approved mockups from /gs:design-shotgun, CEO plans from /gs:plan-ceo-review,
  design review context from /gs:plan-design-review, or from scratch with a user
...
```bash
python "bin/gstack-boot.py" --skill design-html | iex
```
...
# /gs:design-html: Pretext-Native HTML Engine
...
_CEO=$(ls -t ~/.gstack/projects/$SLUG/ceo-plans/*.md | Out-Null | head -1)
[ -n "$_CEO" ] ; echo "CEO_PLAN: $_CEO" ; echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch | Out-Null ; true
_APPROVED=$(ls -t ~/.gstack/projects/$SLUG/designs/*/approved.json | Out-Null | head -1)
[ -n "$_APPROVED" ] ; echo "APPROVED: $_APPROVED" ; echo "NO_APPROVED"
```

```bash
setopt +o nomatch | Out-Null ; true
_VARIANTS=$(ls -t ~/.gstack/projects/$SLUG/designs/*/variant-*.png | Out-Null | head -1)
[ -n "$_VARIANTS" ] ; echo "VARIANTS: $_VARIANTS" ; echo "NO_VARIANTS"
```

```bash
setopt +o nomatch | Out-Null ; true
_FINALIZED=$(ls -t ~/.gstack/projects/$SLUG/designs/*/finalized.html | Out-Null | head -1)
...
If A: tell the user to run /gs:design-shotgun, then come back to /gs:design-html.
...
> C) Run /gs:design-shotgun — jump straight to visual design exploration
> D) Just describe it — tell me what you want and I'll design the HTML live

If A, B, or C: tell the user to run that skill, then come back to /gs:design-html.
...
$B screenshot $env:TEMP\gstack-verify-mobile.png --width 375
$B screenshot $env:TEMP\gstack-verify-tablet.png --width 768
$B screenshot $env:TEMP\gstack-verify-desktop.png --width 1440
```
